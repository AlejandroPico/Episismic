...
const GLOBE_THEME = {
  morning: {
    ...
    oceanLabel: '#1b465b',
    oceanLabelHalo: 'rgba(240,250,255,.92)',
    volcanoFill: '#b94732',
  ...
  afternoon: {
    ...
    oceanLabel: '#eff8fb',
    oceanLabelHalo: 'rgba(18,43,57,.95)',
    volcanoFill: '#ed704d',
...
function politicalOceanGeoJson(): FeatureCollection {
  const features: FeatureCollection['features'] = [];
  // Franjas estrechas evitan un polígono que atraviese de extremo a extremo el antimeridiano.
  for (let west = -180; west < 180; west += 30) {
    const east = west + 30;
    const westEdge = Array.from({ length: 19 }, (_, index) => [west, -90 + index * 10]);
    const eastEdge = Array.from({ length: 19 }, (_, index) => [east, 90 - index * 10]);
    const ring = [...westEdge, ...eastEdge, westEdge[0]];
...
function marineLabelsGeoJson(): FeatureCollection {
  const labels: Array<[string, number, number]> = [
    ...
    ['Mar de los Sargazos', -53, 28], ['Golfo de México', -91, 24],
  ];
...
function addSourceAndLayers(map: maplibregl.Map) {
  ...
  map.addSource('political-ocean', { type: 'geojson', data: politicalOceanGeoJson() as never });
  ...
  map.addLayer({ id: 'political-ocean-fill', type: 'fill', source: 'political-ocean', layout: { visibility: 'none' }, paint: { 'fill-color': GLOBE_THEME.night.ocean, 'fill-opacity': 1 } });
  map.addLayer({
...
export function GlobeView({
  ...
  useEffect(() => {
    ...
    setVisibility(map, ['political-ocean-fill'], mapStyle === 'political');
    ...
    map.setPaintProperty('political-ocean-fill', 'fill-color', palette.ocean);
